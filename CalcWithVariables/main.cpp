...
struct calculator : public qi::grammar<Iterator, int(), boost::spirit::ascii::space_type>
{
    calculator(stack<double>& eval_) : calculator::base_type(expression), eval(eval_) {
            factor =
    ...
    qi::symbols<double>     vars;
    /*qi::rule<ScannerT>      statement, declaration, var_decl,
                            assignment, expression, term, factor;*/
	qi::rule<Iterator, unsigned()> statement, declaration, var_decl,
                            assignment, expression, term, factor;

    //qi::rule<ScannerT> const& start() const { return statement; }
