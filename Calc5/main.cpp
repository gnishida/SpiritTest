...
namespace client
{
    ...
    struct calculator : qi::grammar<Iterator, ascii::space_type>
    {
          ...
          , code(code)
        {
            ...
            expression =
                term
                >> *(   ('+' > term             [push_back(phoenix::ref(code), op_add)])
                    |   ('-' > term             [push_back(phoenix::ref(code), op_sub)])
                    )
                ;

            term =
                factor
                >> *(   ('*' > factor           [push_back(phoenix::ref(code), op_mul)])
                    |   ('/' > factor           [push_back(phoenix::ref(code), op_div)])
                    )
                ;

            factor =
                uint_                           [
                                                    push_back(phoenix::ref(code), op_int),
                                                    push_back(phoenix::ref(code), _1)
                                                ]
                |   '(' > expression > ')'
                |   ('-' > factor               [push_back(phoenix::ref(code), op_neg)])
                |   ('+' > factor)
                ;
